...
const fadeUp: Variants = {
  ...
  show: (d: number = 0) => ({
    ...
    transition: {
      duration: 0.8,
      delay: d,
      ease: [0.22, 1, 0.36, 1] as const, // ✅ penting biar TS ga error
    },
  }),
};
...
function TypingName({ text }: { text: string }) {
  ...
  const steps = useMemo(() => Math.max(18, Math.min(44, text.length + 8)), [text]);
        ...
        <div className="relative p-6 sm:p-10 flex flex-col-reverse lg:flex-row gap-8 lg:items-center lg:justify-between">
          ...
          <div className="max-w-2xl">
            ...
            <motion.p
              ...
              variants={fadeUp}
              custom={0.2}
              className="mt-4 text-zinc-300/90 text-base sm:text-lg leading-relaxed text-justify"
            >
              {PROFILE.role} di{" "}
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <a href={PROFILE.waLink} target="_blank" rel="noreferrer" className="social-btn wa" aria-label="WhatsApp">
                <FaWhatsapp className="social-icon" size={18} />
              </a>
              <a href={PROFILE.socials?.instagram} target="_blank" rel="noreferrer" className="social-btn ig" aria-label="Instagram">
                <FaInstagram className="social-icon" size={18} />
              </a>
              <a href={PROFILE.socials?.facebook} target="_blank" rel="noreferrer" className="social-btn fb" aria-label="Facebook">
                <FaFacebook className="social-icon" size={18} />
              </a>
              <a href={PROFILE.socials?.tiktok} target="_blank" rel="noreferrer" className="social-btn tt" aria-label="TikTok">
          ...
          >
            {/* ✅ FIX: jangan spread floatAnim, pakai animate+transition langsung */}
            <motion.div
              animate={{ y: [0, -10, 0] }}
        ...
        >
          <div className="p-6 sm:p-8 border-b border-white/10">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl sm:text-2xl font-semibold tracking-tight">{ABOUT.headline}</h2>
                <p className="mt-2 text-zinc-300/90 max-w-2xl">{ABOUT.tagline}</p>
              </div>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-zinc-200">{PROFILE.location}</span>
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-zinc-200">{PROFILE.campus}</span>
              <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-zinc-200">Open to internship / freelance</span>
            </div>
          </div>
          ...
          <div className="p-6 sm:p-8">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="lg:col-span-8 order-2">
                <p className="text-zinc-300/90 leading-relaxed max-w-3xl">{ABOUT.summary}</p>

                <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {ABOUT.roles.slice(0, 4).map((r) => (
                    <div key={r.title} className="group rounded-3xl border border-white/10 bg-white/5 p-5 hover:bg-white/10 transition">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-zinc-100 leading-tight">{r.title}</p>

                          <div className="mt-2 flex flex-wrap gap-2">
                            {(r.tags ?? []).slice(0, 2).map((t) => (
                              <span key={t} className="rounded-full border border-white/10 bg-white/5 px-3 py-1 text-[11px] text-zinc-200">
                                {t}
                              </span>
                            ))}
                          </div>
                ...
                <div className="mt-7 flex flex-wrap gap-3">
                  <Link href="/portfolio" className="rounded-2xl px-4 py-2 text-sm font-semibold bg-gradient-to-r from-sky-400 to-violet-400 text-zinc-950 hover:brightness-110 transition">
                    Lihat Projects
                  </Link>
                  <Link href="/gallery" className="rounded-2xl px-4 py-2 text-sm border border-white/10 bg-white/5 hover:bg-white/10 transition">
                    Gallery
                  </Link>
                  <Link href="/contact" className="rounded-2xl px-4 py-2 text-sm border border-white/10 bg-white/5 hover:bg-white/10 transition">
                    Contact
                  </Link>
                </div>
              </div>
